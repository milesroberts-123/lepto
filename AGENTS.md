...
- **`workflow/rules/*.smk`** — 6 rule modules: `kmc.smk`, `panther.smk`, `orthofinder.smk`, `featurecounts.smk`, `degenotate.smk`, `vg_diversity.smk`
- **`workflow/envs/*.yaml`** — conda environment specs (one per tool)
...
- **`pool_sizes.csv` is auto-generated** at Snakefile parse time (line 25-28 of `Snakefile`) from `samples.tsv`. Do not commit or manually edit it.
- **Conda environments are required.** Always use `--use-conda` when running snakemake.
- **Target rules** (entrypoints): `all`, `panther_all`, `orthofinder_all`, `featurecounts_all`, `degenotate_all`, `vg_diversity_all`.
